...
- **Claude Code releases**: updated tracking from v2.1.272 to v2.1.274 with two documented releases and UTC npm publication dates. v2.1.273 adds opt-in LLM gateway hint headers behind `CLAUDE_CODE_GATEWAY_HINT_HEADERS=1`, closes unanalyzable Bash commands skipping the prompt under `permissions.blockReadsOutsideWorkingDirectories` and MDM MCP restrictions being ignored alongside server-managed settings, and fixes auto-compact firing at about half the real window. v2.1.274 adds a critical memory warning and `CLAUDE_CODE_MCP_STARTUP_WAIT_MS`, and fixes the endless "unexpected tool_use_id" retry loop, `/goal` loss on resume and the 5-minute Streamable HTTP MCP timeout. Recorded six behavior changes: v2.1.273 reverts the v2.1.268 Read/Edit deny check on unanalyzable Bash lines and makes auto mode on Bedrock, Vertex and Foundry use the local classifier by default; v2.1.274 skips `"type": "sdk"` MCP entries outside an SDK host, moves Bedrock, Vertex, Foundry and telemetry-disabled installs to the v2 MCP client, switches `/code-review` to inline prompts for untuned models and leaves Git LFS files as pointers in plugin clones. Synchronized the bundled MCP release data (unpublished package 1.3.4, no further bump), the three `llms.txt` mirrors and the landing `releases.ts`. Guide version remains 3.43.0.

- **Routing Claude Code through a gateway** (`guide/ops/api-gateway.md`, `guide/ops/subscription-strategy.md`, `guide/ecosystem/ai-ecosystem.md`, `guide/ecosystem/local-vs-cloud-inference.md`): integrated Nicolas Martignole's September 2026 two-month review of Back Market's move from the Anthropic API to OpenRouter for roughly 245 developers, cross-checked against his July 2026 account already cited in the guide. Narrowed the existing provider-allowlist caveat without removing it: about 70 providers were available, about ten are in use, and five are now named, but the auto-generated transcript garbles the size of the approved shortlist, and "in use" is not the same list as "approved". Added the liability boundary the July account states explicitly, one aggregator contract simplifies procurement but does not transfer responsibility for each routed provider, and the rule that ten providers mean ten data policies to verify. Calibrated the model-allowlist budget tiers with the reported $200 per developer per month, its self-serve $20 extension gated by a console-reading form, and committee-approved project workspaces. Added the $87,000 to $23,000 monthly figure with the explicit caveat that model substitution and a per-developer budget landed together, so neither can be credited alone, and that no quality or time measurement was published. Resolved a contradiction between pages: the ai-ecosystem section filed `ANTHROPIC_BASE_URL` gateways under community workarounds "not recommended for production" while api-gateway.md recommends the same mechanism; it now separates a contracted commercial gateway from a reverse-engineered proxy, with the proxy warning kept verbatim. Added a routing table in local-vs-cloud-inference.md pointing to the page that owns each question. Each figure appears on one page only. Renamed anchor `alternative-providers-community-workarounds` to `pointing-claude-code-at-another-backend` in the table of contents, `machine-readable/reference.yaml` and its MCP mirror.

- **The post-merge half of the software factory** (`guide/workflows/agentic-software-factories.md`, `docs/resource-evaluations/2026-09-15-openai-agentic-software-factory.md`): integrated Gergely Orosz's September 2026 account of OpenAI's agentic pipeline (score 4/5). Every level in the existing orientation map ended at a merged pull request, and the guide had no coverage of incident-response agents, latency-regression agents or per-change deployment agents. Added section 6 documenting the four post-merge stages with the gate owner for each, three transferable patterns with their costs (risk classification as review routing, per-change observability and its unaddressed retention problem, an incident agent that proposes but never executes), and the one loop the account leaves open, incident to permanent fix. Added OpenAI's reported delivery-load figure to the unbounded-velocity-trap section as a second operator datapoint alongside Anthropic's CI account, with the shared caveat that neither organization publishes a defect, escape or change-failure rate next to its throughput numbers. Recorded that the circulating pipeline diagram is the journalist's rendering, not an OpenAI artifact, and that several of its apparent omissions are contradicted by the body text. Added the corresponding `deep_dive` key and section-map anchor to `machine-readable/reference.yaml` and its MCP mirror.

- **Claude Code releases**: updated tracking from v2.1.269 to v2.1.272 with three documented releases and UTC npm publication dates. v2.1.271 adds per-command `allowed_domains` for Bash, PowerShell and Monitor in auto mode with sandboxing, `omitClaudeMd` in agent frontmatter, `--accept-command <sha256>` for plugin install and update, fast mode in Remote sessions and mouse support in `/config`. Recorded three behavior changes: Monitor watches lose the no-timeout `persistent` option in favor of a bounded deadline, inline `!` shell commands in skills and slash commands follow default-mode permission rules in auto mode, and dynamic workflows default to the small size on Pro plans with the medium guideline dropping from 15 to 10 agents. v2.1.270 fixes a v2.1.269 regression that made read-only git commands prompt in long sessions; v2.1.272 is maintenance only. Synchronized the bundled MCP release data (package 1.3.4), the three `llms.txt` mirrors and the landing `releases.ts`. Guide version remains 3.43.0.

- **CI test selection as an agent primitive** (`guide/core/agent-harness.md`, `guide/ops/team-metrics.md`, `guide/workflows/agentic-software-factories.md`, `docs/resource-evaluations/2026-09-14-anthropic-ci-test-impact-analysis.md`): integrated Anthropic's own account of scaling test impact analysis under agent-driven CI load (score 4/5). Added a new "Test Selection as an Agent Primitive" subsection to `agent-harness.md` section 4, a scoped datapoint (tests grew 10x, CI jobs grew 25x over six months) to the DORA-in-an-AI-context deployment-frequency discussion in `team-metrics.md`, and a from-the-inside counterpoint, verification infrastructure buckling rather than code, to the unbounded-velocity-trap section in `agentic-software-factories.md`. Added the corresponding `deep_dive` key to `machine-readable/reference.yaml` and its MCP mirror.
...
- **Translation provenance check needs full history in CI** (`.github/workflows/index-integrity.yml`): the provenance validator reads recorded blobs with `git show <commit>:<path>`, which the default single-commit checkout cannot resolve, so every declared source commit reported as unavailable. Set `fetch-depth: 0` for the integrity job.

- **Index Integrity gate restored** (`machine-readable/reference.yaml`, MCP mirrors, `llms.txt` mirrors, `mcp-server/{server.json,package-lock.json}`, `machine-readable/mcp-product.json`): the positional-reference backlog blocked CI with 65 broken entries. Integrated the anchor declarations from the machine-readable architecture work, applied 46 anchor-declared repairs, and manually re-pointed the remaining ten duplicate-slug or drifted entries to their verified headings (0 broken, 0 hard validator failures). Aligned the three `llms.txt` mirrors with tracked Claude Code v2.1.269 and re-rendered registry metadata, lockfile and product manifest for MCP package 1.3.3, which still declared 1.3.1.
...
- **Multica control-plane profile and catalog integration** (`guide/ecosystem/{agentic-tools.md,agent-harness-landscape.md,third-party-tools.md}`, `guide/ultimate-guide.md`, `docs/resource-evaluations/2026-09-05-multica.md`, machine-readable catalog and indexes): reviewed `multica-ai/multica` at commit `7a438bd5b8bf39afd54259a7eb0971390e50a8ef` and classified it as an adjacent issue, chat, and scheduling control plane over external coding-agent CLIs, not a runtime that owns the inner model-tool loop. The profile documents the server and PostgreSQL coordination tier, local daemon execution, 26 advertised integrations, queued-work recovery, server-stored run data, daemon-account permission boundary, Git enforcement boundary, and restricted source-available licence. Current GitHub metadata was checked on 2026-09-09 at 49,348 stars, 6,375 forks, and release v0.4.41. The Agent Harness Map now contains 193 projects: 160 pinned upstream projects plus 33 guide supplements, including 16 adjacent control planes, while retaining 42 strict runtimes. Upstream CI for the audited commit passed; no local application run or real-agent end-to-end task was performed.

- **Coding agent interface map and Warp Agent CLI profile** (`guide/ecosystem/{agent-harness-landscape,agentic-tools,ai-ecosystem}.md`, `machine-readable/agent-harnesses*`, generator, schema, tests, indexes, and MCP mirrors): replaced the empty 42-row interface comparison with a controlled `cli`/`tui`/`ide`/`desktop`/`web`/`chat` taxonomy and complete interface coverage for every strict runtime. Upstream tags now map deterministically, five untagged upstream runtimes use commit-pinned official overrides, and the build fails when a strict runtime has no interface. Removed the provider-strategy column because all 42 values were unestablished. Added a sourced Warp Agent CLI profile covering standalone terminal use, PTY multiplexing, multi-agent coordination, and optional cloud handoff, with vendor-claim and data-boundary caveats. Reframed the former IDE section around three separate axes: interface, execution location, and loop ownership. Renamed the current Windsurf entry to Devin Desktop while preserving Windsurf in the dated 2025 migration case study. Added deep links to `reference.yaml` and all `llms.txt` mirrors. Catalog and page regression suites pass, with 78 and 16 tests respectively.
- **Start / Build / Scale adoption navigation** (`guide/roles/adoption-approaches.md`): added a decision layer for controlled first use, repeatable workflows, and governed operation under team or volume constraints. The section keeps L0-L5 as a separate autonomy scale, makes Scale optional, and links to the portfolio's public Adoption Path.
...
- **MCP 1.3.1 Registry identity correction** (`mcp-server/package.json`, generated registry metadata, product manifest, and statistics collector): aligned the case-sensitive package and server name with the GitHub OIDC namespace granted by the registry, `io.github.FlorianBruniaux/claude-code-guide`. npm 1.3.0 remains immutable with the lowercase metadata value; 1.3.1 is the published, registry-compatible patch.
- **Public MCP snapshot execution fixed** (`mcp-server/scripts/generate-public-runtime-snapshot.mjs`): runs the exact npm package outside its source directory so `npx` cannot resolve the local package without its installed binary. The refreshed snapshot records the observed 1.3.1 handshake and capability names.

## [3.43.0] - 2026-09-01

<!-- mcp-product:start -->
- **MCP product documentation, registry metadata, and aggregate release gate** (`machine-readable/mcp-product.json`, `mcp-server/server.json`, deterministic renderers, package and guide documentation): rendered current package 1.3.4 capabilities into marker-delimited surfaces, corrected Claude Code and Codex install commands plus project `.mcp.json` configuration, documented bundled versus network and local-write behavior including `get_digest`, and published only the 5 companion command files that exist. The canonical technical guide records the public npm package, observed runtime, and repository candidate as separate evidence states when their versions differ, and documents installation, architecture, privacy, offline behavior, troubleshooting, and dated metrics. The renderers validate every target before writing. The manual release workflow checks the live JSON-RPC contract, a clean consumer install of the exact packed archive, manifest, generated documentation, registry metadata, tests, and dry-run package contents through `npm run release:check`. It prepares checksum-pinned Registry validation before approval and an approval-gated publication sequence for `io.github.FlorianBruniaux/claude-code-guide`. npm trusted-publisher binding remains an external prerequisite, and a required reviewer on the `mcp-production` environment remains an external prerequisite; repository code does not verify either setting. The reproducible monthly npm, GSC, and GA4 dashboard limits Google queries to the two MCP page URLs and aggregate metrics; missing Google access remains unavailable rather than zero. The separate npm public-runtime snapshot command starts the exact version returned by npm and writes sanitized capability names and counts to machine-readable/mcp-public-runtime.json; it excludes descriptions, resource URIs, prompt arguments, call arguments, results, user content, local paths, and user and request identifiers. Dependency security: @modelcontextprotocol/sdk 1.30.0; `npm audit --omit=dev` reported zero production vulnerabilities on 2026-08-31; one low-severity development-only esbuild advisory remains in the full audit.

<!-- mcp-stats:start -->
- **MCP public statistics snapshot** (`machine-readable/mcp-stats.json`): snapshot `2026-09-26T14:41:39Z` for npm 1.3.3; since launch 2026-02-28 through 2026-09-25: 9,992 downloads; year to date 2026-01-01 through 2026-09-25: 9,992 downloads; trailing 30 complete UTC days 2026-08-27 through 2026-09-25: 1,452 downloads; trailing 7 complete UTC days 2026-09-19 through 2026-09-25: 123 downloads. The package is published. Download counts are not users, active installations, sessions, or executions.
<!-- mcp-stats:end -->
<!-- mcp-product:end -->
...
- **Added awesome-claude-code-hooks to Complementary ecosystem** (`README.md`): added a row for [loqimean/awesome-claude-code-hooks](https://github.com/loqimean/awesome-claude-code-hooks), a curated list of Claude Code hooks, to the resource discovery table (renamed from "Complementary Resources" to "Complementary ecosystem" in the v3.43.0 README restructure; re-applied here after a merge conflict dropped the row).
...
- **Absolute machine paths removed from the tracked hook allowlist** (`.claude/settings.json`): the `env.ALLOWED_PATHS` block listed eight of Florian's local repository paths in a public repository. The list already lives in the untracked `.claude/settings.local.json`, which is where machine-specific paths belong, so `dangerous-actions-blocker.sh` still resolves the same allowlist. Documented here because the July entry below records adding the landing repo to this block, and that entry no longer describes the tracked file.
...
- **ORF (Open Reasoning Format) re-evaluated, revised 3/5 to 2/5 after an adversarial challenge pass** (`docs/resource-evaluations/orf-open-reasoning-format.md`, `docs/resource-evaluations/README.md`): source-level evaluation of Guillaume Laforge's file-based cross-session memory format for coding agents ([blog post](https://glaforge.dev/posts/2026/07/21/open-reasoning-format/), 2026-07-21). An agent records operational learnings as Markdown playbooks with YAML frontmatter under `./experiences/`, then reloads them next session to skip dead ends it already paid for. The design bet is explicit: drop the embedding/vector retrieval that ReasoningBank (Google, [arXiv:2509.25140](https://arxiv.org/abs/2509.25140), verified) uses, and retrieve by filename plus one-line frontmatter descriptions read by the LLM, trading semantic recall for zero infrastructure and Git-committable files. **The initial 3/5 was corrected down to 2/5**: a skeptical review found the novelty overstated. The flagship "new pattern", three-tier progressive disclosure for memory retrieval, is already documented in `memory-systems.md` §3.1 (claude-mem, "Progressive disclosure (3 layers to save tokens)", verified), and §2.2-2.3 (Auto Memory + Auto Dream) is already a native file-based, no-vector, index-and-prune memory system, the exact shape ORF pitched as its differentiator. Novelty dropped 4/5 to 2/5, recompute (4+2+3+1+3)/5 = 2.6, rounds to 2. What is genuinely new is narrower: git-committable, per-domain, agent-authored 5-section failure/fix playbooks shared via `git commit experiences/`. **Evidence quality scored 1/5**: the "66.7% to 100% SWE-bench Lite" figure is 3 tasks with one flipped, "-52% steps" is a single scenario, and the author states plainly it is "by no means a scientific evaluation"; no benchmark number is citable. The author self-reports "haven't used the ORF skill in anger yet". **Decision revised to minimal mention**: do not add a tool entry to §3 (catalog bloat, core already covered), the one real win is citing ReasoningBank (arXiv 2509.25140) since `grep -rli "reasoningbank" guide/` returns zero, optionally a one-paragraph §4 note on git-committable playbooks as a team-sharing path. Do not recommend for production. The prior commit (`69ad760`) had also omitted the README index row; this pass adds it. Guide integration still pending.
